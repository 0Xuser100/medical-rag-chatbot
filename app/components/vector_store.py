        ...
        if os.path.exists(DB_FAISS_PATH):
            ...
            if not os.path.exists(index_path) or not os.path.exists(pkl_path):
                error_message = CustomException(
                    f"Vector store files missing. Expected: {index_path} and {pkl_path}"
                )
            ...
            try:
                ...
                logger.info(
                    "Vector store may be incompatible with current embedding model"
                )
                # Try to recreate the vector store
                return recreate_vector_store()
        else:
            error_message = CustomException(
                "No vector store found at path: " + DB_FAISS_PATH
            )
            logger.error(str(error_message))
            raise error_message
